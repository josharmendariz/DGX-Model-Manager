...
_DASHBOARD_TEMPLATE = (_APP_DIR / "dashboard.html").read_text()

_engine_pills = "".join(f'    <div class="pill" id="pill-{k}"><div class="dot"></div><span>{e["name"]}</span></div>\n' for k, e in _ENGINES.items())
_engine_nav_items = "".join(f'    <div class="nav-item" id="nav-{k}" onclick="switchTab(\'{k}\')">\n      <span class="nav-icon">{e["icon"]}</span>{e["name"]}\n    </div>\n' for k, e in _ENGINES.items())
_log_tab_buttons = "".join(f'            <button class="btn btn-sm log-tab-btn{" active" if i == 0 else ""}" id="eng-tab-{k}" onclick="switchEngineLog(\'{k}\')">{e["name"]}</button>\n' for i, (k, e) in enumerate(_ENGINES.items()))
_mem_alert_pct = str(_ALERT_THRESHOLDS["memory_percent"])
_engine_tabs = "".join(f'''    <div class="tab" id="tab-{k}">
      <div class="page-hdr">
...
''' for k, e in _ENGINES.items())
_settings_engine_cards = "".join(f'''      <div class="card" id="svc-{k}-card" style="margin-bottom:8px">
...
''' for k, e in _ENGINES.items())
_debug_engine_cards = "".join(f'''      <div class="card" style="margin-bottom:8px;padding:12px 14px">
...
HTML = (_DASHBOARD_TEMPLATE
    .replace("<!--DGX:ENGINE_PILLS-->", _engine_pills)
    .replace("<!--DGX:NAV_ITEMS-->", _engine_nav_items)
    .replace("<!--DGX:ENGINE_TABS-->", _engine_tabs)
    .replace("<!--DGX:SETTINGS_ENGINE_CARDS-->", _settings_engine_cards)
    .replace("<!--DGX:DEBUG_ENGINE_CARDS-->", _debug_engine_cards)
    .replace("<!--DGX:LOG_TAB_BUTTONS-->", _log_tab_buttons)
    .replace("__MEM_ALERT_PCT__", _mem_alert_pct)
    .replace("<!--DGX:ENGINES_JS-->", _engines_js)
)
